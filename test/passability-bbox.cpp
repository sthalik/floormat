...
void test_hole_zero_bbox_is_constructible()
{
    auto w = world();
    constexpr chunk_coords_ ch{0, 0, 0};
    auto& c = w[ch];
...
#if 0
    // Crashes today: add_holes_from_chunk asserts min != max on enabled holes
    c.mark_passability_modified();
    c.ensure_passability();
#endif
}

void test_can_place_pass_mode_zero_bbox()
{
    const auto invalid_scenery = loader.invalid_scenery_atlas();
    auto w = world();
    constexpr chunk_coords_ ch{0, 0, 0};
    auto& c = w[ch];
    c.mark_modified();
    ...
    scenery_proto p;
    p.atlas = invalid_scenery.proto->atlas;
    p.subtype = generic_scenery_proto{};
    p.bbox_size = Vector2ub{0, 0};
    p.pass = pass_mode::pass;
    fm_assert(c.can_place_object(p, local_coords{4, 4}));
}
...
void Test::test_passability_bbox()
{
    ...
    test_hole_zero_bbox_is_constructible();
    test_can_place_pass_mode_zero_bbox();
}
