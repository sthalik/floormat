...
{
    // combined permutation: {3, 1, 2, 0}
    auto base = Quads::texcoords_at(pos, size, image_size, false, false);
    auto got  = Quads::texcoords_at(pos, size, image_size, true, true);
    fm_assert_equal(base[3], got[0]);
    fm_assert_equal(base[1], got[1]);
    fm_assert_equal(base[2], got[2]);
    fm_assert_equal(base[0], got[3]);
}
...
void test_second_set_of_values()
{
    ...
    fm_assert_equal(base[3], both[0]);
    fm_assert_equal(base[1], both[1]);
    fm_assert_equal(base[2], both[2]);
    fm_assert_equal(base[0], both[3]);
}
