...
struct merge_state
{
    ...
    Array<run> runs; // k entries, long-lived/reserved
    Array<uint32_t> tree; // k entries (inner nodes store loser_index), long-lived/reserved
    // output goes to scratch arrays (vertex, texture, depth) of size n
};
