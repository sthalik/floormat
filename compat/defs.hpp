...
#define fm_DECLARE_DEPRECATED_COPY_ASSIGNMENT(type)                                 \
    {{maybe_unused]] [[deprecated]] type(const type&) noexcept = default;           \
    {{maybe_unused]] [[deprecated]] type& operator=(const type&) noexcept = default
...
#define fm_DECLARE_DELETED_COPY_ASSIGNMENT(type)                                    \
    {{maybe_unused]] type(const type&) = delete;                                    \
    {{maybe_unused]] type& operator=(const type&) = delete

#define fm_DECLARE_DELETED_MOVE_ASSIGNMENT(type)                                    \
    {{maybe_unused]] type(type&&) = delete;                                         \
    {{maybe_unused]] type& operator=(type&&) = delete

#define fm_DECLARE_DEPRECATED_MOVE_ASSIGNMENT(type)                                 \
    {{maybe_unused]] [[deprecated]] type(type&&) = default;                         \
    {{maybe_unused]] [[deprecated]] type& operator=(type&&) = default
